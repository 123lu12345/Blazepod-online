    ...
    def game_loop():
        ...
        while time.time() < end_time:
            ...
            if mode == "shapes":
                ...
                # Send target pod with target color
                socketio.emit("blink", {"target": target_pod, "color": target_color, "mode": mode})

                # Send distractor colors to other pods
                ...
                # Track allowed pod
                allowed_touch_pod = target_pod
                waiting_for_touch["focus_allowed"] = allowed_touch_pod
                ...
                word_options = ["Rot", "Blau", "Grün", "Gelb", "Pink", "Orange", "Weiß", "Grau", "Violett"]
                ...
                color = random.choice(colors)
                socketio.emit("blink", {"target": None, "color": color, "mode": mode})
            ...
            socketio.emit("blink", {"target": None})
            gray_pause = random.uniform(0.3, pause) if random_timing else pause
            time.sleep(gray_pause)
