    ...
    def game_loop():
        ...
        while time.time() < end_time:
            ...
            if mode == "shapes":
                ...
                def pod_touched_handler(data):
                    pod_id = data.get("pod_id")
                    if pod_id in waiting_for_touch:
                        waiting_for_touch[pod_id]["touched"] = True

                socketio.on_event("pod_touched", pod_touched_handler)
                ...
                while time.time() < end_time:
                    ...
                    waiting_for_touch[target] = { "touched": False }
                    start = time.time()

                    while not waiting_for_touch[target]["touched"] and not stop_event.is_set():
                        time.sleep(0.05)
                ...
                avg_reaction = round(total_reaction / score, 2) if score > 0 else 0
                socketio.emit("game_finished", {"score": score, "reaction": avg_reaction})
                time.sleep(1)
                socketio.emit("game_stopped")
            elif mode == "stroop":
