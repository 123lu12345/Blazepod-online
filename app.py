        ...
        def reaction_loop():
            ...
            while time.time() < end_time:
                ...
                for pod in active_pods:
                    ...
                    color_assignments[pod] = color
                    socketio.emit("blink", {"target": pod, "color": color, "mode": mode})

                correct_pod = next((pod for pod, color in color_assignments.items() if color == target_color), None)
                if correct_pod:
                    waiting_for_touch[correct_pod] = True

                touched = False
                while not touched and not stop_event.is_set():
                    for pod in active_pods:
                        if waiting_for_touch.get(pod) == False:
                            touched = True
                            break
                    time.sleep(0.05)
            ...
            time.sleep(1)
            socketio.emit("game_stopped")
        Thread(target=reaction_loop).start()
        return
