...
def start_game(data):
    ...
    light_logic = data.get("lightLogic", "random")
    ...
    def game_loop():
        ...
        while time.time() < end_time:
            ...
            if not active_pods:
                break
            if mode == "classic" and light_logic == "all-at-once":
                for pod_id in active_pods:
                    socketio.emit("blink", {"target": pod_id, "color": random.choice(colors), "mode": mode})
                time.sleep(interval)
                socketio.emit("blink", {"target": None})
                time.sleep(pause)
                continue
