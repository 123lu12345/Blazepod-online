...
def start_game(settings):
    ...
    if mode == "classic":
        ...
        while time.time() - start_time < duration:
            selected_pod = random.choice(active_pods)
            emit("show_color", {"color": "red"}, to=selected_pod)
            time.sleep(blink_interval)
            emit("show_color", {"color": "black"}, to=selected_pod)
            time.sleep(pause_between)

    elif mode == "reaktionsspiel":
        print("Starte Reaktionsspiel-Modus")
        total_points = 0
        ...
        while time.time() - start_time < duration:
            ...
            for pid in active_pods:
                emit("show_color", {"color": current_colors[pid]}, to=pid)

            clicked_pod = wait_for_click_or_timeout(active_pods, blink_interval)
            if clicked_pod == target_pod:
                total_points += 1
                print(f"✔ Richtiger Klick: {clicked_pod}")
            else:
                print(f"✖ Falscher Klick oder keine Reaktion")

            for pid in active_pods:
                emit("show_color", {"color": "black"}, to=pid)
            time.sleep(pause_between)

        for pid in active_pods:
            emit("show_score", {"score": total_points}, to=pid)

def wait_for_click_or_timeout(pods, timeout):
    # Platzhalter für echte Logik. Simuliert Timeout ohne Klick.
    time.sleep(timeout)
    return None
