        ...
        while time.time() < end_time:
            ...
            if mode == "shapes":
                ...
                if len(active_pods) < 1:
                    break
                target = random.choice(active_pods)  # Zielpod in Rot
                distraction_pods = [p for p in active_pods if p != target]
                distract_colors = [c for c in colors if c != "red"]
                
                # Zielpod (rot)
                socketio.emit("blink", {"target": target, "color": "red", "mode": mode})

                # Ablenkungspods (nicht-rot)
                for pod in distraction_pods:
                    color = random.choice(distract_colors) if distract_colors else "blue"
                    socketio.emit("blink", {"target": pod, "color": color, "mode": mode})

                # Warten auf Touch des roten Pods
                waiting_for_touch[target] = True
                while waiting_for_touch.get(target, False) and not stop_event.is_set():
                    time.sleep(0.05)
