...
def start_game(data):
    ...
    def game_loop():
            ...
            if mode == "shapes":
                ...
                target_color = "red"  # vorerst fest, später auswählbar
                distractors = [c for c in colors if c != target_color]
                target = random.choice(active_pods)
                for pod in active_pods:
                    if pod == target:
                        socketio.emit("blink", {"target": pod, "color": target_color, "mode": mode})
                    else:
