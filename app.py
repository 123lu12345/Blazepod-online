    ...
    def game_loop():
        ...
        while time.time() < end_time:
            ...
            if mode == "shapes":
                ...
                time.sleep(0.5)
                continue
            elif mode == "focus-touch":
                if len(active_pods) < 2:
                    break  # Fokus-Modus macht nur Sinn mit mindestens 2 Pods

                # Wähle zufällig ein Pod als Ziel (Rot)
                target = random.choice(active_pods)
                distraction_pods = [p for p in active_pods if p != target]

                # Schicke explizit das ROT-Signal an den Zielpod
                socketio.emit("blink", {
                    "target": target,
                    "color": "red",
                    "mode": mode
                ...
                # Alle anderen Pods bekommen eine zufällige Ablenkfarbe (≠ Rot)
                distract_colors = [c for c in colors if c != "red"]
                for pod in distraction_pods:
                    if distract_colors:
                        distract_color = random.choice(distract_colors)
                    else:
                        distract_color = "blue"  # Fallback
                    socketio.emit("blink", {
                        "target": pod,
