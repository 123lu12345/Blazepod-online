    ...
    def game_loop():
        ...
        while time.time() < end_time and not stop_event.is_set():
            ...
            if mode == "fokus":
                ...
                blink_data = [(target, target_color)]
                other_pods = [p for p in active_pods if p != target]
                non_target_colors = [c for c in DEFAULT_COLORS if c != target_color]
                for pod in other_pods:
                    color = random.choice(non_target_colors)
                ...
                socketio.emit("blink", {"target": target, "color": color, "mode": "touch-switch"})
                waiting_for_touch[target] = True
                while waiting_for_touch.get(target, False) and not stop_event.is_set():
                    time.sleep(0.05)
                socketio.emit("blink", {"target": None})
