...
def disconnect():
    ...
    touch_count = 0
    reaction_times = []
    global touch_count, reaction_times, all_reactions
    touch_count = 0
    reaction_times = []
    all_reactions = []
...
print(f"[DEBUG] Spielzeit in Sekunden: {duration}")
    ...
    def game_loop():
        ...
        print(f"🕒 Spiel startet mit Dauer: {duration} Sekunden")
        print(f"🕒 Endzeit (Timestamp): {end_time}")
        ...
        if mode == "touch-switch" and touch_count > 0:
            ...
            socketio.emit("touch_results", {
                "count": touch_count,
                "average": round(avg_reaction, 2),
                "details": [round(rt, 2) for rt in all_reactions]
            })
        if mode == "touch-switch" and touch_count > 0:
            avg_reaction = sum(reaction_times) / len(reaction_times)
