    ...
    def game_loop():
        ...
        while time.time() < end_time:
            ...
            if mode == "shapes":
                ...
                # Emit target pod with target color
                socketio.emit("blink", {"target": target_pod, "color": target_color, "mode": mode})

                # Emit distractor pods with random non-target colors
                distractor_colors = [c for c in colors if c != target_color]
                for pod in other_pods:
                    distractor_color = random.choice(distractor_colors) if distractor_colors else "black"
                    socketio.emit("blink", {"target": pod, "color": distractor_color, "mode": mode})
