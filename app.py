...
def start_classic_mode(duration, interval, pause, colors):
    ...
    while not stop_event.is_set() and time.time() - start_time < duration:
        if not pod_ids: break
        active_pod = random.choice(pod_ids)
        color = random.choice(colors)
        socketio.emit("show_color", {"pod_id": active_pod, "color": color})
        time.sleep(interval)
...
def start_game(data):
    ...
    if mode == "classic":
        start_classic_mode(duration, interval, pause, colors)
