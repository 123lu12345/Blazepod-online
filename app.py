...
def start_game(data):
    stop_event.clear()
    for pod in connected_pods.values(): waiting_for_touch[pod] = True
    ...
    def game_loop():
        ...
        if mode == "reaktionsspiel":
            ...
            while time.time() < end_time:
                ...
                # Wähle Farben aus erlaubter Liste
                if not colors:
                    break
                non_target_colors = [c for c in colors if c != target_color]
                if not non_target_colors:
                    non_target_colors = colors

                target_pod = random.choice(active_pods)
                pod_colors = {}
                for pod in active_pods:
                    if pod == target_pod:
                        pod_colors[pod] = target_color
                    else:
                        pod_colors[pod] = random.choice(non_target_colors)

                # Sende Farben an Pods
                for pod, color in pod_colors.items():
                    socketio.emit("blink", {"target": pod, "color": color})

                # Warte auf Berührung
                touched = None
                while not stop_event.is_set():
                    for sid, pod in connected_pods.items():
                        if pod in waiting_for_touch and not waiting_for_touch[pod]:
                            touched = pod
                            break
                    if touched:
                        break
                ...
                # Punkte zählen
                if touched == target_pod:
                    score += 1

                # Alle Pods schwarz schalten
                socketio.emit("blink", {"target": None})
                time.sleep(pause)
