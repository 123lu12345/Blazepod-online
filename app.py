...
def handle_disconnect():
    connected_pods.pop(request.sid, None)
    emit("update", list(set(connected_pods.values())), broadcast=True)
...
def handle_start(data):
    ...
    # Countdown (10s) auf allen Geräten
    for i in range(10, 0, -1):
        socketio.emit("countdown_tick", {"value": i})
        time.sleep(1)

    # Spielstart
    thread = Thread(target=start_classic_mode, args=(duration, interval, pause, colors, pod_ids))
    thread.start()
...
def start_classic_mode(duration, interval, pause, colors, pod_ids):
